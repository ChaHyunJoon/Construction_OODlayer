...
function simulate!(
    ...
    for _ in 1:sim_batch_size
        ...
        ConstructionBots.respec_step!(env)
        ...
        if !isnothing(factory_vis.vis)
            if !isempty(newly_updated) || sim_params.update_anim_at_every_step
                ...
                if process_animation_tasks
                    ...
                    update_tuple = (sim_process_data.iter, nothing,
        ...
        if process_animation_tasks &&
           (length(update_steps) >= process_updates_interval || project_stop_bool)
            ...
            if block_save_anim
                anim = ConstructionBots.AnimationWrapper(0, factory_vis.vis)
...
function get_object_vtx(scene_tree, obstacles, max_cargo_height, num_layers, robot_d)
    ...
    while !found_size
        ...
        if length(pts) * num_layers >= num_objects
            ...
            if cnt > 2_000
                error("[RUNAWAY] get_object_vtx grid-search loop exceeded 2000 expansions — never found enough legal points. legal_pts=$(length(pts)), need=$num_objects, n_obstacles=$(length(inflated_obstacles))")
            end
    ...
    println("[DIAG] get_object_vtx: grid search done after cnt=$cnt expansions"); flush(stdout)
