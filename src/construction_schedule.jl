...
function populate_schedule_sub_graph!(sched, parent::AssemblyComplete, model_spec, scene_tree, id_map)
    ...
    while !(step_node === nothing)
        _diag_bs += 1
        if _diag_bs > 10_000
            error("[RUNAWAY] populate_schedule_sub_graph! build-step loop exceeded 10000 iterations for parent $(summary(node_id(parent_assembly))) — likely a cycle in get_previous_build_step / model_spec build steps. step_node=$(summary(node_id(step_node)))")
        end
        # CloseBuildStep
...
function generate_staging_plan!(scene_tree, sched;
    ...
    for node in node_iterator(sched, topological_sort_by_dfs(sched))
        ...
        elseif matches_template(OpenBuildStep, node)
            _diag_step += 1
            println("[DIAG] staging_plan: build step #$(_diag_step) ($(summary(node_id(node))))"); flush(stdout)
    ...
    println("[DIAG] staging_plan: finished $(_diag_step) build steps; selecting assembly start configs..."); flush(stdout)
    ...
    println("[DIAG] staging_plan: select_assembly_start_configs done"); flush(stdout)
    ...
    for start_node in node_iterator(sched, topological_sort_by_dfs(sched))
        ...
        if matches_template(AssemblyNode, node)
            ...
            while !isempty(part_ids_left)
                _diag_outer += 1
                println("[DIAG] assembly_start: $(summary(node_id(assembly))) outer iter=$(_diag_outer), parts_left=$(length(part_ids_left))"); flush(stdout)
                if _diag_outer > 10_000
                    error("[RUNAWAY] select_assembly_start_configs_layered! outer loop exceeded 10000 iterations for $(summary(node_id(assembly))) — part_ids_left not shrinking ($(length(part_ids_left)) left). No part fits per iteration.")
                end
                ...
                while staging_radius - ring_radius > 1e-6
                    _diag_ring += 1
                    if _diag_ring % 50 == 0
                        println("[DIAG] assembly_start: ring-align loop iter=$(_diag_ring), staging_radius=$staging_radius, ring_radius=$ring_radius"); flush(stdout)
                    end
                    if _diag_ring > 1_000
                        error("[RUNAWAY] select_assembly_start_configs_layered! ring-align loop exceeded 1000 iterations (each calls the solver) — staging_radius not converging. staging_radius=$staging_radius, ring_radius=$ring_radius")
                    end
...
function solve_ring_placement_problem(θ_des, r, R, rmin=0.0;
    ...
    while sum(half_widths) * 2 >= 2.0 * Float64(π)
        _diag_R_iter += 1
        if _diag_R_iter % 50 == 0
            println("[DIAG] solve_ring_placement: R-grow loop iter=$(_diag_R_iter), R=$R, sum(half_widths)*2=$(sum(half_widths)*2)"); flush(stdout)
        end
        if _diag_R_iter > 100_000
            error("[RUNAWAY] solve_ring_placement_problem R-grow loop exceeded 100000 iterations — R not converging. R=$R, r=$r, sum(half_widths)*2=$(sum(half_widths)*2)")
        end
