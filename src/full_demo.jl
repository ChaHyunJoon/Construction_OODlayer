...
function run_lego_demo(;
    ...
    return_env_before_sim::Bool=false, # RESPEC: return the fully-built PlannerEnv just before simulation (for verify-gate tests)
    ...
    if process_animation_tasks
        ...
        anim = ConstructionBots.AnimationWrapper(0, visualizer)
    ...
    if process_animation_tasks
        ...
        if !isnothing(anim)
            try
                _nclips = length(anim.anim.clips)
                _nframes = 0
                for (_p, _clip) in anim.anim.clips
                    for _tr in values(_clip.tracks)
                        _nframes = max(_nframes, length(_tr.events))
                    end
                end
