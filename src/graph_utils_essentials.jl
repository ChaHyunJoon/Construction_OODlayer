...
const _CACHE_TIMESTAMP_COUNTER = Ref{Float64}(0.0)
function _next_cache_timestamp()
    _CACHE_TIMESTAMP_COUNTER[] += 1.0
    return _CACHE_TIMESTAMP_COUNTER[]
end

CachedElement(element, flag) = CachedElement(element, flag, _next_cache_timestamp())
...
Set the time stamp of n.
"""
function set_time_stamp!(n::CachedElement, t=_next_cache_timestamp())
...
"""
Base.copy(e::CachedElement) = CachedElement(e.element, copy(is_up_to_date(e)), _next_cache_timestamp())

Base.convert(::Type{CachedElement{T}}, e::CachedElement) where {T} = CachedElement{T}(get_element(e), is_up_to_date(e), _next_cache_timestamp())
