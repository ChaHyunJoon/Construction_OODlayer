# =============================================================================
# run_parallel_openworld.ps1 -- parallel oracle data generation (OPEN-WORLD variant).
#
# Fork of run_parallel.ps1 for the 2026-07-27 open-world / kind-agnostic work.
# ONE change to the unit mix: `zoneblk0.9` (the middle zone severity, redundant with
# 0.7/1.1) is swapped for `fault_sp1`. Rationale: the verify.py S3 diagnostic reports
#   "spare_count: fault data covers only {0,12} -> grade DS_SPARES=0,1,3,6"
# and `spare_count` is a KIND-TRANSFERABLE physical descriptor -- exactly the kind of
# feature the kind-agnostic featurize redesign must lean on instead of the `kind_*`
# one-hots. Adding the sp=1 level makes the axis informative ({0,4,12}) at ZERO extra
# compute. Everything else (pool size, resumability, Common env) is unchanged.
#
# ---- original header ----
...
$Units = @(
    ...
    @{ tag="fault_sp1";   env=@{ DS_KINDS="fault";     DS_SPARES="1" } },
